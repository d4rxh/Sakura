...
export const LoginPromo: React.FC = () => {
  ...
  const FeatureRow = ({ title, guest, premium }: { title: string, guest: boolean | string, premium: boolean | string }) => (
    <div className="grid grid-cols-[1.5fr_1fr_1fr] py-4 border-b border-[#FF6B9D]/15 items-center last:border-0">
  ...
  return (
    <div className="min-h-full bg-[#0A000F] text-white font-sans flex flex-col relative overflow-y-auto pb-10">
      
      {/* Background Ambience */}
      <div className="fixed top-0 right-0 w-full h-[500px] bg-gradient-to-b from-[#FF6B9D]/20 to-transparent opacity-40 z-0 pointer-events-none"></div>

      {/* Header */}
      <div className="z-10 flex items-center justify-between p-4 sticky top-0 bg-transparent">
          <button onClick={() => navigate(-1)} className="bg-[#0A0A0A] p-2 rounded-full hover:bg-[#0A000F] transition-colors">
      ...
      <div className="z-10 flex-1 flex flex-col px-4 md:px-12 pt-2 pb-12 max-w-4xl mx-auto w-full">
          ...
          {/* Comparison Table */}
          <div className="bg-[#130018] rounded-xl p-5 md:p-8 mb-8 relative overflow-hidden">
              <div className="absolute top-0 right-0 w-32 h-32 bg-accent/10 rounded-full blur-3xl -translate-y-1/2 translate-x-1/2"></div>
              
              <div className="grid grid-cols-[1.5fr_1fr_1fr] mb-6 pb-4 border-b border-[#FF6B9D]/15">
          ...
          <div className="flex flex-col gap-4 items-center">
              <button 
                ...
                className="w-full md:w-2/3 font-bold py-4 rounded-full text-white text-lg hover:scale-105 transition-transform"
              >
                  LOG IN TO SYNC
              </button>
              <button 
